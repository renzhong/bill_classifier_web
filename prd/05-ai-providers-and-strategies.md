...
- 在 Pipeline 中加入 `ai_classify` 步骤时绑定某条 active 策略
- 调试时给一条样例输入，看 AI 返回什么类别

## 页面
- `/settings/ai/credentials`：provider/model_name/api_key/base_url/enabled
- `/settings/ai/strategies`：name + 多行 strategy_text + 绑定 credential + active 开关
...
- `ai_strategies(id, user_id, name, strategy_text, active, credential_id, ts)`

## 验收
- api_key 写入后从 DB 取出已加密；解密失败抛错而不裸露
- `POST /ai/test` 能复用 ai_classify 的实际调用路径
- Pipeline 中 `ai_classify` 步骤未配 active strategy 时给出明确错误
