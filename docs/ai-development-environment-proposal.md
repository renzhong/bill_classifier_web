...
- 从新任务界面选择 Cloud 和已关联业务仓库的环境，以远端 `main` 为起点，核对能读取 `AGENTS.md`、Spec Kit 技能与 constitution；云端环境不依赖本机 `.venv`。
...
| 步骤 | 具体工作 | 可核对结果 |
|---|---|---|
| 0 | 保留已登记的业务项目与现有云端环境；本机编码任务选择 Codex 原生 Worktree，Cloud 任务选择业务环境，Local 启动时使用仓库后备规则 | 工作树来自业务仓库；云端从远端检出；不把后备规则误认为项目级自动开关 |
