...
预期分类为 5 条成功、1 条未分类；退款后金额为 70 元。
同一来源重复上传 CSV 或 Excel，账单总数应保持 6 条；分别上传微信和支付宝则共 12 条。
样本日期固定在 2026 年 1—2 月，查看报表时选择对应月份。

真实 AI 验证：先在页面配置 AI 凭据，绑定「固定样本分类规则」，再启用分类步骤后上传。
已经上传的账单可在账单详情重新分类；重复上传会去重，不会重新调用模型。
默认数据不含真实 API key，不会自行调用付费模型。

自动测试使用同一份 SQL 和上传文件，并模拟模型返回，验证提示词组装、返回值解析、分类回写、退款金额及去重。
这验证的是程序流程；真实模型的分类质量需与 `expected.json` 单独对比。
